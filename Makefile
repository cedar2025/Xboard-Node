...
build:
	go build -ldflags "$(LDFLAGS)" -tags "$(BUILD_TAGS)" -o xboard-node ./cmd/xboard-node
	go build -ldflags "$(LDFLAGS)" -o xbctl ./cmd/xbctl

# Build for Linux amd64
build-linux:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -tags "$(BUILD_TAGS)" -o xboard-node-linux-amd64 ./cmd/xboard-node
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -o xbctl-linux-amd64 ./cmd/xbctl

# Build for Linux arm64
build-linux-arm64:
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -tags "$(BUILD_TAGS)" -o xboard-node-linux-arm64 ./cmd/xboard-node
...
# Run tests
test:
	go test -v -race -count=1 -tags "$(TEST_TAGS)" ./...
